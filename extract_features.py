...
__all__ = ['get_time_domain_features', 'get_frequency_domain_features',
           ...
           "get_csi_cvi_features", "get_sampen","DFA"]
...
def get_time_domain_features(nn_intervals: List[float]) -> dict:
    ...
    nni_70 = sum(np.abs(diff_nni) > 70)
    pnni_70 = 100 * nni_70 / length_int
    nni_100 = sum(np.abs(diff_nni) > 100)
    pnni_100 = 100 * nni_100 / length_int
...
def _get_features_from_psd(freq: List[float], psd: List[float], vlf_band: namedtuple = VlfBand(0.0033, 0.04),
    ...
    Returns
    ---------
    freqency_domain_featudfares : dict
        Dictionary containing frequency domain features for HRV analyses. There are details
...
def DFA(pp_values, lower_scale_limit, upper_scale_limit):
    ...
    scaleDensity = (upper_scale_limit - lower_scale_limit) +1 # scales DFA is conducted between lower_scale_limit and upper_scale_limit
    #scaleDensity = 50 # scales DFA is conducted between lower_scale_limit and upper_scale_limit
    m = 1 # order of polynomial fit (linear = 1, quadratic m = 2, cubic m = 3, etc...)
    scales = np.linspace(lower_scale_limit, upper_scale_limit, scaleDensity)
    scales = np.round(scales).astype(int)
    X = np.linspace(1, scales.max(), scales.max())
    ...
    for s in scales:
        ...
        x = pp_values
        N = len(x)
        shape = [int(s), int(np.floor(N/s))]
        i = shape[0]
        y_n = np.cumsum(x - np.mean(x))
        #print(count)
        #print(s)
        #if (lower_scale_limit == 16) and (ip == 880):
        #    if s==38:
        #        import pdb; pdb.set_trace()
        Y_n = y_n[:N - N % i].reshape(i, (N - N % i) // i, order = "F")
        fit_line = np.polyfit(X[:i],Y_n, 1)
        xcut = np.tile(X[:i], (int(shape[1]), 1))
        Y_fit = np.polyval(fit_line, xcut.T)
        arr = Y_fit - Y_n
        rms = np.sqrt(np.mean(arr * arr, axis =0))
        if len(rms) == 0:
            F[count] = (F[count-1]   + F[count-2])/2
        else:
            F[count] = np.power((1 / (int(np.floor(N/s)))) * np.sum(np.power(rms, 2)), 1/2)
        count +=1
