...
def plot_poincare(nn_intervals: List[float], selected_intervals: List[float], 
                              plot_sd_features=True, filepath: str = "path"):
    """
    ...
    ax11 = selected_intervals[:-1]
    ax22 = selected_intervals[1:]

    # compute features for ellipse's height, width and center
    dict_sd1_sd2 = get_poincare_plot_features(selected_intervals)
    sd1 = dict_sd1_sd2["sd1"]
    sd2 = dict_sd1_sd2["sd2"]
    mean_nni = np.mean(selected_intervals)
